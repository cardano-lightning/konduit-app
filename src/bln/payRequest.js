...
import * as bolt11 from "./bolt11.js";
 ...
 * @returns {bolt11.Invoice | object} A parsed object with a 'type' field ('bolt11' or 'lnurl').
...
export const parsePayRequest = (rawText) => {
  ...
  if (rawInvoice) {
    return bolt11.decode(rawInvoice);
  }
...
function extractLnurl(text) {
  ...
  try {
    const decoded = bech32.decode(rawLnurl.toLowerCase(), false); // 2000 = large max length
